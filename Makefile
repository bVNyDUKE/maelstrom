...
test: build
	docker run --name $(DOCKER_CONTAINER_NAME) -it --rm $(DOCKER_IMAGE_NAME) ./maelstrom test -w echo --bin mlstrm --nodes n1 --time-limit 10 --log-stderr
...
broadcast: build
	docker run --name $(DOCKER_CONTAINER_NAME) -it --rm $(DOCKER_IMAGE_NAME) ./maelstrom test -w broadcast --bin mlstrm --time-limit 20 --rate 10 --node-count 1
